...
export class PackagesController {
  ...
  @ApiOperation({
    summary: 'Generate a share token for a package',
    description:
      'Creates an HMAC-SHA256 signed share token encoding whether pricing should be shown. The token is tamper-proof — modifying it will invalidate the signature.',
  })
    ...
    schema: {
      ...
      properties: {
        showPricing: {
          type: 'boolean',
          example: false,
          description: 'Whether the share link should reveal pricing',
        },
      },
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        token: 'eyJwYWNrYWdlSWQiOiI....<payload>.<hmac_sig>',
        shareUrl: 'https://app.oaknpine.com/packages/:id?st=<token>',
      },
    },
  })
  generateShareToken(
    @Param('id') id: string,
    @Body('showPricing') showPricing: boolean,
    @CurrentTenant() tenantId: string,
  ) {
    const { token } = this.packagesService.generateShareToken(
      id,
      showPricing,
      tenantId,
    );
  ...
  // ==================== PUBLIC PACKAGE ENDPOINT (token-aware) ====================

  @Public()
  @Get('public/:id')
  @ApiOperation({
    summary: 'Get package publicly (with optional share token)',
    description:
      'Returns package data for public viewing. If a valid share token (st) is supplied and it encodes showPricing=false, all pricing fields are stripped from the response and pricingHidden=true is set.',
  })
  @ApiParam({ name: 'id', description: 'Package UUID' })
  @ApiQuery({
    name: 'st',
    required: false,
    description: 'HMAC-signed share token generated by POST /:id/share-token',
  })
  @ApiResponse({
    status: 200,
    description: 'Package details (pricing may be hidden based on token)',
    type: Package,
  })
  @ApiResponse({ status: 401, description: 'Tampered or invalid share token' })
  @ApiResponse({ status: 404, description: 'Package not found' })
  getPublicPackageById(
    @Param('id') id: string,
    @Query('st') shareToken?: string,
  ) {
    return this.packagesService.getPublicPackageById(id, shareToken);
  }
